...
from graphene_sqlalchemy.types import SQLAlchemyObjectType, sort_argument_for_object_type
...
def init_custom_connection_field(
    ...
    filters = {
        **custom_connection_field.filters,
        **generated_filters
    }
...
def filter_factory(base_filter_class: FilterSet, sqla_model: DeclarativeMeta,
                   custom_filters_path: str = None) -> FilterSet:
    ...
    try:
        ...
        logging.debug("Can't get {} from {} - auto generate".format(filter_class_name, custom_filters_path))
        generated_fields = {
...
def node_factory(custom_connection_field,
    ...
    try:
        ...
        logging.debug("Can't get {} from {} - auto generate".format(node_name, custom_schemas_path))
        meta = type(
            'Meta',
            (object,),
            {
...
def connections_factory(node: SQLAlchemyObjectType, custom_connection) -> Connection:
    ...
    return custom_connection.create_type(
        connection_name, node=node
    )
...
def _get_table_args_key(sqla_model: DeclarativeMeta, key: str, default=''):
    ...
    if isinstance(sqla_model.__table_args__, dict):
        ...
        value = next((o.get(key) for o in sqla_model.__table_args__ if isinstance(o, dict)), default)
    return value
...
class QueryObjectType(ObjectType):
    ...
    def __init_subclass_with_meta__(
        ...
        for model in custom_connection_field.filters:
            ...
            fields.update(
                {
                    inflection.underscore(model.__name__): graphene.relay.Node.Field(node),
